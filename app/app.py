...
comids = pd.read_csv("./data/comids_mod.csv", index_col=0)["0"].values
...
async def start_model(app, loop):
    ...
    for model in model_collection.models.values():
        checkpoint = CheckPoint(model, timedelta=3600.0)
    ...
    for model in model_collection.models.values():
        ...
        if model_sites:
            ...
            # basin_measurements = basin_measurements.loc[:, ~basin_measurements.columns.duplicated()].copy()  # noqa
...
async def reach_forecast(request, reach_id):
    ...
    json_output = {"timestamp__utc": timestamp_utc, "streamflow__cms": streamflow_cms}
