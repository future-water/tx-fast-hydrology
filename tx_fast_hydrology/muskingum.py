...
class Muskingum:
    ...
    def datetime(self, new_datetime):
        try:
            ...
            raise TypeError("New datetime must be of type `pd.Timestamp`") from e
        try:
            assert new_datetime.tz == datetime.timezone.utc
        except Exception as e:
            raise ValueError("New datetime must be UTC.") from e
    ...
    def timedelta(self, new_timedelta):
        try:
            ...
            raise TypeError("New timedelta must be of type `pd.Timedelta`") from e
    ...
    def o_t(self, new_o_t):
        try:
            ...
            raise TypeError("New `o_t` must be convertible to float64 np.ndarray") from e
    ...
    def load_model(self, obj, load_optional=True):
        ...
        try:
            ...
            raise ValueError(f"Model field must contain fields {required_fields}") from e
        try:
            ...
            raise TypeError("Typing of input arrays is incorrect.") from e
        try:
            assert (
                obj["startnodes"].size
                == obj["endnodes"].size
            ...
            raise ValueError("Arrays are not the same length") from e
    ...
    def create_state_space(self, overwrite_old=True):
        ...
        A, B = self.muskingum_matrix(A, B, startnodes, endnodes, alpha, beta, chi, gamma, indegree)
        self.A = A
        self.B = B

    def muskingum_matrix(self, A, B, startnodes, endnodes, alpha, beta, chi, gamma, indegree):
        m = startnodes.size
        n = endnodes.size  # noqa
        indegree_t = indegree.copy()
        for k in range(m):
            startnode = startnodes[k]
            endnode = endnodes[startnode]
            while indegree_t[startnode] == 0:
                alpha_i = alpha[startnode]  # noqa
                beta_i = beta[startnode]  # noqa
                chi_i = chi[startnode]
    ...
    def step_matrix(self, p_t_next: np.ndarray, num_iter: int = 1, inc_t: bool = False):
        raise NotImplementedError
        A = self.A
        B = self.B
        dt = self.dt
        timedelta = self.timedelta  # noqa
    ...
    def step_iter(self, p_t_next: np.ndarray, timedelta=None):
        ...
        i_t_next, o_t_next = _ax_bu(
            sub_startnodes, endnodes, alpha, beta, chi, gamma, i_t_prev, o_t_prev, p_t_next, indegree
        )
        self.o_t_next = o_t_next
        self.o_t_prev = o_t_prev
        self.i_t_next = i_t_next
    ...
    def step(self, p_t_next: np.ndarray, timedelta=None):
        """
        ...
        **kwargs,
        ...
        if start_time is None:
            ...
            try:
                ...
                raise ValueError(
                    "If `start_time` is specified, initial state `o_t_init` must be provided."
                ) from e
        ...
        while self.datetime < end_time:
            ...
            p_t_next = interpolate_sample(
                float(next_timestep.value),
    ...
    def simulate(self, dataframe, start_time=None, end_time=None, o_t_init=None, **kwargs):
        ...
        """
        return self.simulate_iter(
            dataframe, start_time=start_time, end_time=end_time, o_t_init=o_t_init, **kwargs
        )
    ...
    def split(self, indices, name=None, create_state_space=True):
        ...
        startnode_indices = np.asarray(
            [np.flatnonzero(self.startnodes == i).item() for i in indices]
        )
        ...
        for component in range(n_components):
            ...
            if (startnode_index >= 0) and (endnode_index >= 0):
                ...
                downstream_model.i_t_next[downstream_index] -= upstream_model.o_t_next[
                    upstream_index
                ]
                # Fill in sink and source objects
                connection = Connection(
                    upstream_model, downstream_model, upstream_index, downstream_index
                )
...
class Connection:
    def __init__(
        self, upstream_model, downstream_model, upstream_index, downstream_index, name=None
    ):
...
class ModelCollection:
    ...
    def datetime(self):
        ...
        (collection_datetime,) = set([model.datetime for model in self.models.values()])
    ...
    def dump_model_collection(self, file_path, model_file_paths={}, dump_optional=True):
        ...
        model_collection_info = {"models": models, "connections": connections}
...
def load_model_file(file_path, load_optional=True):
    ...
    if load_optional:
        ...
        obj = {k: v for k, v in obj.items() if k not in required_fields}
        return obj
...
def dump_model_file(obj, file_path, dump_optional=True):
    ...
    with open(file_path, "w") as f:
        if dump_optional:
            ...
            to_dump = {k: v for k, v in obj.items() if k in required_fields}
        json.dump(to_dump, f, cls=ModelEncoder)


def load_nhd_geojson(file_path: str) -> dict[str, Any]:
    with open(file_path, "r") as f:
        d = json.load(f)
...
def load_model_collection(file_path, load_optional=True):
    ...
    for connection_name, connection_dict in connections.items():
        ...
        connection = Connection(
            upstream_model, downstream_model, upstream_index, downstream_index, name=connection_name
        )
        upstream_model.sinks.append(connection)
        downstream_model.sources.append(connection)
