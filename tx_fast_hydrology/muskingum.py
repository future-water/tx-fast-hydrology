...
class ExpectationMaximization(KalmanSmoother):
    def __init__(self, model, measurements, Q_cov, R_cov, P_t_init, regularization=0.):
        super().__init__(model, measurements, Q_cov, R_cov, P_t_init)
    ...
    def solve_parameters(self):
        ...
        # TODO: Should this be inputs rather than measurements?
        p_t = (inputs.reindex(model.reach_ids, 
        ...
        T3 = (0.5 * o_hat_next - 0.5 * i_hat_next 
            - 0.5 * i_hat_prev + 0.5 * o_hat_prev - p_t[1:])
        # Inner blocks
        G = np.zeros((m, m))
        G[0, 0] = (T3**2).sum()
        G[1:, 0] = np.column_stack([(T1 * T3).sum(axis=0), (T2 * T3).sum(axis=0)]).ravel()
        G[0, 1:] = np.column_stack([(T1 * T3).sum(axis=0), (T2 * T3).sum(axis=0)]).ravel()
        blocks = np.column_stack([(T1**2).sum(axis=0), (T1 * T2).sum(axis=0), (T1 * T2).sum(axis=0), (T2**2).sum(axis=0)])
...
class ExpectationMaximizationIO(KalmanSmootherIO):
    def __init__(self, model, measurements, Q_cov, R_cov, P_t_init, temp_file, regularization=0.):
        super().__init__(model, measurements, Q_cov, R_cov, P_t_init, temp_file)
        self.regularization = regularization
        self.K = model.K.copy()
        self.X = model.X.copy()
        self.G = None
